...
export class MessageService {
  ...
  showMessageWarning(message, keep_message = false): void {
    this.keepAfterNavigationChange = keep_message;
    const msg: {class: string, text: string } = {class: 'alert-warning', text: message};
    this.message_emitter.emit(msg);
  }

  showMessageDanger(message,  keep_message = false): void {
    this.keepAfterNavigationChange = keep_message;
    const msg: {class: string, text: string} = {class: 'alert-danger' , text: message};
    this.message_emitter.emit(msg);

  }

  showMessageSuccess(message,  keep_message = false): void {
    this.keepAfterNavigationChange = keep_message;
    const msg: {class: string, text: string} = {class: 'alert-success', text: message};
    this.message_emitter.emit(msg);
  }
